...
router.post("posts/:postId/comments", [auth], async (req, res, next) => {
  try {
    ...
    // Check if the post exists and belongs to the same course course: courseId
...
router.get("/posts/:postId/comments", [auth], async (req, res, next) => {
  try {
    const postId = req.params.postId;
    const userId = req.user.id; // Assuming you have implemented user authentication
...
router.get("/posts/:postId/comments/Mobile", [auth], async (req, res, next) => {
  try {
    const postId = req.params.postId;
    // Assuming you have implemented user authenticatio
    // Find the post and check if it belongs to the same course
    const post = await Post.findOne({ _id: postId });
